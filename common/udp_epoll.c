...
{
    for (int i = 0; i < MAX; i++) {
        if (rteam[i].online && strcmp(request->name, rteam[i].name) == 0)
            return 1;
        if (bteam[i].online && strcmp(request->name, bteam[i].name) == 0)
            return 1;
    }
    return 0;
}
...
int udp_accept(int fd, struct User *user)
{
    ...
    if (request.team) {
        DBG(GREEN "Info" NONE " : " BLUE "%s login on %s:%d  <%s>\n", request.name, inet_ntoa(client.sin_addr),
            ntohs(client.sin_port), request.msg);
    } else {
