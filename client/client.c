...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "h:p:t:m:n:")) != -1) {
        switch (opt) {
            ...
            case 'n':
                ...
                break;
            default: fprintf(stderr, "Usage : %s [-hptmn]!\n", argv[0]); exit(1);
        }
    }
    ...
    DBG("<" GREEN "Conf Show" NONE "> : server_ip = %s, port = %d, team = %s, name = %s\n%s", server_ip, server_port,
        request.team ? "BLUE" : "RED", request.name, request.msg);
    ...
    sendto(sockfd, &request, sizeof(request), 0, (struct sockaddr *)&server, len); // HERE
    ...
    if (retval < 0) {
        ...
        if (ret != sizeof(response) || response.type) {
            DBG(RED "Error" NONE "The Game Server refuse your login.\n\tThis may be helpful:%s\n", response.msg);
            exit(1);
        }
    } else {
